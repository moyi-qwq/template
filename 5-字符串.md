...
char text[maxn], mode[maxn];
int pfxf[maxn];
int main()
{
	cin.getline(text + 1, maxn);
	cin.getline(mode + 1, maxn);
	int lt = strlen(text+1), lb = strlen(mode+1);
	int j = 0;
	for(int i = 2; i <= lb; i++)
	{
		while(j && mode[j+1] != mode[i]) j = pfxf[j];
	...
	for(int i = 1; i <= lt; i++)
	{
		while(j>0&&mode[j+1]!=text[i])
			j = pfxf[j];
		if(mode[j+1]==text[i]) j++;
		if(j==lb){cout<<i-lb+1<<endl;j=pfxf[j];}
	}
	for(int i = 1; i <= lb; i++)
	{
		cout<<pfxf[i]<<" ";
